...
class TranslatorBot(ActivityHandler):
    ...
    def _teams_file_download_card(self, file_name: str, download_url: str, unique_id: str) -> Attachment:
        ...
        # MIME YANG BENAR (jika salah → Unknown attachment type)
        return Attachment(
    ...
    async def _handle_attachments(self, turn_context: TurnContext, to_lang: str = "en"):
        ...
        try:
            ...
            # 7) Kirim hasil: coba File Download Card → fallback ke link bila ditolak
            ...
            await turn_context.send_activity("Hasil terjemahan (berlaku 4 jam):")
            for b in blobs:
                ...
                try:
                    file_card = self._teams_file_download_card(file_name, url, unique_id=b.name)
                    ...
                    await turn_context.send_activity(url)  # fallback supaya user tetap dapat file
